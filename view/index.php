...
<!-- Hero Section -->
    <div class="bg-gradient-to-r from-green-500 to-green-600 py-12 md:py-20">
        <div class="container mx-auto text-center text-white px-4">
            ...
            <p class="text-lg md:text-xl mb-8">Skills for your present (and future). Get started with us.</p>
            <a href="login.php" class="bg-white text-green-600 px-6 py-3 mt-4 rounded-lg font-semibold hover:bg-gray-100">Explore now</a>
            <?php }?>
        </div>
    </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            foreach($courses as $course){
                if($course['status'] == 'active'){
                    ...
                    if($count == 0){
                        if($_SESSION){
                            echo "<a href='detailsCourse.php?course_id=".$course['course_id']."&type=".$course['type']."&user_id=".$course['teacher_id']."' class='justify-self-end text-lg font-medium text-gray-100 py-2 bg-green-500 text-center rounded-lg transition-all duration-300 ease-in-out hover:text-gray-50 hover:bg-green-600 hover:scale-105 hover:shadow-lg'>S'inscrire</a>";
                        }else{
                        ...
                        echo "<a href='detailsCourse.php?course_id=".$course['course_id']."&type=".$course['type']."&user_id=".$course['teacher_id']."' class='justify-self-end text-lg font-medium text-gray-100 py-2 bg-green-500 text-center rounded-lg transition-all duration-300 ease-in-out hover:text-gray-50 hover:bg-green-600 hover:scale-105 hover:shadow-lg'>Accéder au cours</a>";
                    }
                    echo "</div>";
                    echo "</div>";
                }
